...
fetch('/produto-salvo/api').then((response)=>{
    ...
    data.map((item)=>{
        ...
        `
            <div id="produto">
                    ...
                    <div id="preco-produto">${item.preco}</div> 
    ...
    produtos.map((item)=>{
        ...
        item.querySelector("a").addEventListener('click',(e)=>{
            ...
            let valor = parseFloat(janela_modal_carrinho.querySelector('#valor').innerHTML)
            let valor_inicial = parseFloat(item.querySelector('#preco-produto').innerHTML)
            let quantidade = 1            
