...
export async function salvar_carrinho(req,res){
    ...
    try{
        if(req.session.login){            
            ...
            //Verifica se já existe o produto no carrinho, caso exista, adiciona preço e quantidade no produto já existente
            if(verificar.rows != ''){
                ...
                await cliente.query(`INSERT INTO carrinho (id_produto, produto, categoria, preco_unitario, preco_carrinho, quantidade, id_usuario, nome_usuario) VALUES
                (${produto.rows[0].id},'${produto.rows[0].produto}','${produto.rows[0].categoria}',${produto.rows[0].preco},${parseFloat(preco_carrinho)},${parseInt(quantidade)},${_usuario.rows[0].id},'${_usuario.rows[0].nome}')`)
            }           
